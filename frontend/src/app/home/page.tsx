    ...
    await fetch("/api/v1/case/history", {
      ...
      .then(async (res: Response) => {
        if (res.ok) {
          ...
          setIsFetching(false);
          ToastMessage(
            { message: "Failed to fetch cases. Please try again later." },
            "error",
          );
        }
      })
          ...
          >
            ...
            <Button
              ...
              onClick={() => {
                ToastMessage(
                  {
                    message: `Navigating to case ${caseItem.title}`,
                    description: "Please wait! redirecting...",
                  },
                  "info",
                );
                setTimeout(
                  () => router.push(`/home/case/${caseItem.case_id}`),
                  500,
                );
              }}
    ...
    <div className="flex flex-col items-center justify-start min-h-screen p-6 md:p-10 lg:p-16 bg-gray-50">
      ...
      <div className="mb-8">
        <h1 className="text-3xl font-bold">
          Greetings, <span className="text-blue-600">{fullName}</span>
        </h1>
      </div>
      <div className="w-full max-w-4xl bg-white rounded-xl shadow-lg p-6">
        ...
        <div className="mt-4">
          <div className="mt-4">
            ...
            >
              <Tab value={"Open"} className="font-medium">
                Opened
              </Tab>
              <Tab value={"Resolved"} className="font-medium">
                Resolved
              </Tab>
              <Tab value={"Aborted"} className="font-medium">
                Aborted
              </Tab>
            </TabList>
          </div>
